...
class TestSignupUser:
    ...
    @patch('modules.auth_service.validate_dob')
    @patch('modules.auth_service.validate_fullname')
    def test_signup_success(self, mock_fullname, mock_dob, mock_phone, mock_cognito):
        """Test successful user signup"""
        # Mock validation functions to return True
        mock_fullname.return_value = True
        mock_dob.return_value = True
    ...
    @patch('modules.auth_service.get_cognito_client')
    @patch('modules.auth_service.validate_phone_format')
    def test_signup_invalid_phone(self, mock_phone, mock_cognito):
        ...
        assert 'phone' in result['message'].lower() or 'invalid' in result['message'].lower()
    
    @patch('modules.auth_service.get_cognito_client')
    @patch('modules.auth_service.validate_phone_format')
    @patch('modules.auth_service.validate_dob')
    @patch('modules.auth_service.validate_fullname')
    def test_signup_cognito_user_exists(self, mock_fullname, mock_dob, mock_phone, mock_cognito):
        """Test signup when user already exists in Cognito"""
        # Mock validation functions
        mock_fullname.return_value = True
        mock_dob.return_value = True
        mock_phone.return_value = True
        
        # Mock Cognito client to raise UsernameExistsException
        mock_client = MagicMock()
